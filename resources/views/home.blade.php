    ...
    <main class="mx-auto w-full max-w-2xl px-4 pb-4 pt-10 sm:px-6 sm:pt-14">
        ...
        <header class="relative text-center">
            <div class="pointer-events-none absolute inset-x-0 -top-10 flex justify-center" aria-hidden="true">
                <x-ornament.arabesque class="w-64 text-gold-400 opacity-[0.06] sm:w-80" />
            </div>

            <div class="reveal relative flex flex-col items-center gap-3">
                <x-emblem class="w-24 text-gold-400 sm:w-28" />

                <h1 class="gold-text text-[2rem] font-black leading-tight tracking-tight sm:text-5xl">
                    {{ $settings['cafe_name'] ?? 'کافه صاحبقرانیه' }}
                </h1>

                <p class="latin text-[0.625rem] text-gold-300/85 sm:text-xs">
                    {{ $settings['cafe_name_latin'] ?? 'Saheb Gharaniyeh Cafe' }}
                </p>

                <x-ornament.divider class="mt-1 max-w-[16rem] sm:max-w-xs" />

                <p class="text-sm text-gold-100/90 sm:text-base">
        ...
        <section class="mt-7 sm:mt-9" aria-labelledby="menu-heading">
            <div class="reveal flex flex-col items-center gap-2">
                <h2 id="menu-heading" class="gold-text text-lg font-bold sm:text-2xl">منوی کافه</h2>
                <p class="text-[0.6875rem] text-cream-dim">یک دسته را انتخاب کنید</p>
                       ...
                       aria-label="مشاهده {{ $card->cardTitle() }}">
                        <span class="relative z-2 flex items-center gap-4 px-4 py-5 sm:px-6 sm:py-6">
                            ...
                            <span class="min-w-0 flex-1">
                                <span class="block text-base font-bold text-gold-100 sm:text-xl">
                                    {{ $card->cardTitle() }}
                                </span>

                                @if ($card->card_subtitle)
                                    <span class="mt-1 block text-[0.6875rem] leading-relaxed text-cream-dim sm:text-xs">
                                        {{ $card->card_subtitle }}
                                    </span>
                                @endif
                                ...
                                @if ($latin = $card->card_latin ?? $card->latin_name)
                                    ...
                                    <span class="latin mt-1.5 block text-[0.5rem] text-gold-400/70 sm:text-[0.5625rem]">
                                        {{ $latin }}
                                    </span>
                                @endif
                            </span>

                            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-gold-400/35 bg-night-900/60 text-gold-200 transition-transform duration-500 group-hover:-translate-x-1">
                                <x-icon.chevron dir="start" class="h-4 w-4" />
                            </span>
                        </span>

                        {{-- hairline top glow --}}
                        <span class="absolute inset-x-0 top-0 h-px bg-gradient-to-l from-transparent via-gold-300/45 to-transparent"></span>
                    </a>
                @endforeach
            </div>
            ...
            <div class="reveal mt-6 text-center" style="--reveal-delay: 420ms">
                <a href="{{ route('menu') }}"
                   class="inline-flex items-center gap-2 rounded-full border border-gold-400/30 px-5 py-2.5 text-xs font-semibold text-gold-100 transition hover:border-gold-300/60 hover:bg-gold-900/30">
                    مشاهده کل منو
        ...
        @if ($intro = $settings['intro'] ?? null)
            <x-frame class="reveal mt-9 px-5 py-6 sm:mt-11 sm:px-8 sm:py-7">
                <p class="text-center text-[0.8125rem] leading-[2] text-cream/85 sm:text-sm sm:leading-[2.1]">
                    {{ $intro }}
                </p>

                <div class="mt-5 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[0.6875rem] text-cream-dim">
                    @if ($hours = $settings['working_hours'] ?? null)
                        <span class="inline-flex items-center gap-1.5">
                            <span class="h-1 w-1 rounded-full bg-gold-400"></span>{{ $hours }}
                        </span>
                    @endif
                    @if ($address = $settings['address'] ?? null)
                        <span class="inline-flex items-center gap-1.5">
                            <span class="h-1 w-1 rounded-full bg-gold-400"></span>{{ $address }}
                        </span>
                    @endif
                </div>
            </x-frame>
